...
def checkupdates():
    ...
    try:
        versionfile = urllib2.urlopen('https://raw.githubusercontent.com/rajeshmajumdar/PloitKit/master/screenshots/version.txt').read()
        if float(versionfile) > float(__version__):
            ...
            lbl.configure(text='''You already have an updated PloitKit.''')
